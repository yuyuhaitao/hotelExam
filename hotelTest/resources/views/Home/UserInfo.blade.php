    ...
    <body>
        ...
        <div class="container width80 pt20">
            <div class="user-face" onclick="alertWin('头像上传','内容',300,200)" style="z-index:100">
               <img src="{{$data['u_img']}}" alt="" width="150" height="150" /> 
          
            </div>
    

            <div class="user-list">
                <span class="user-name">{{$data['u_name']}}</span>
                <span class="user-integer">积分:<i>{{$data['u_point']}}</i></span>
                <span class="user-integer">余额:<i>{{$data['u_money']}}</i></span>
            </div>

            <ul class="user-function-list">
                 <li onclick="javascript:location.href='Home_UserFoot';">我的足迹</li>
                <li onclick="javascript:location.href='Home_UserChong';">进行充值</li>
                <li onclick="javascript:location.href='Home_UserColl';">我的收藏</li>
                    <li onclick="tui()">退出登录</li>
        ...
        <script>
           function alertWin(title, msg, w, h){
                    var content ="<form action='Home_UserImg' method='post' enctype='multipart/form-data'>";
                    content += "<br/><br/><input type='file' name='file' id='file' value='浏览'/>";  
                    content += "<input type='hidden' name='u_id' value="+{{$data['u_id']}}+"/>";  
                    ...
                    function tui(){
                          if(window.confirm('是否退出登录？')) {
                            window.location.href="Home_UserSxh";       
                    }
                }
